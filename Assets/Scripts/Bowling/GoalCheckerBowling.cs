...
public class GoalCheckerBowling : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            pinsPosition.Add(pins[i].transform.localPosition);
            Debug.Log(pinsPosition[i]);
        }
        ballPos = GameObject.FindGameObjectWithTag("bowlingBall").transform.localPosition;
    ...
    public IEnumerator CountPoints()
    {
        ...
        {
            if (pin.transform.localRotation != Quaternion.identity)
            {
    ...
    private void ResetPins()
    {
        for (int i = 0; i < pins.Count; i++)
        {
            pins[i].transform.localRotation = Quaternion.identity;
            pins[i].transform.localPosition = pinsPosition[i];
    ...
    {
        GameObject.FindGameObjectWithTag("bowlingBall").transform.localPosition = ballPos;
